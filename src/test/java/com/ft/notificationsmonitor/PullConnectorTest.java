...
public class PullConnectorTest {
    ...
    public void testSuccessfulRequestIsSentToMatcher() {
        new JavaTestKit(sys) {{
            ...
            final TestActorRef<PullConnector> pullConnectorRef = TestActorRef.create(sys, PullConnector.props(mockHttp, Collections.singletonList(probe.getRef())), "pullConnector1");
    ...
    public void testUnsuccessfulRequestIsHandled() {
        new JavaTestKit(sys) {{
            PullHttp mockHttp = Mockito.mock(PullHttp.class);
            final JavaTestKit probe = new JavaTestKit(sys);
            final TestActorRef<PullConnector> pullConnectorRef = TestActorRef.create(sys, PullConnector.props(mockHttp, Collections.singletonList(probe.getRef())), "pullConnector2");
